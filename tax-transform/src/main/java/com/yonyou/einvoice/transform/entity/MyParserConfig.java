...
public class MyParserConfig extends ParserConfig {
    ...
    public static MyParserConfig getGlobalInstance() {
        return global;
    }

    public static MyParserConfig global                = new MyParserConfig();
    ...
    public boolean                                          compatibleWithJavaBean = MyTypeUtils.compatibleWithJavaBean;

    {
        ...
        for (int i = 0; i < AUTO_TYPE_ACCEPT_LIST.length; i++) {
            hashCodes[i] = MyTypeUtils.fnv1a_64(AUTO_TYPE_ACCEPT_LIST[i]);
        }
    ...
    public MyParserConfig(boolean fieldBase){
        this(null, null, fieldBase);
    }

    public MyParserConfig(ClassLoader parentClassLoader){
        this(null, parentClassLoader, false);
    }

    public MyParserConfig(ASMDeserializerFactory asmFactory){
        this(asmFactory, null, false);
    }

    private MyParserConfig(ASMDeserializerFactory asmFactory, ClassLoader parentClassLoader, boolean fieldBased){
        this.fieldBased = fieldBased;
        if (asmFactory == null && !ASMUtils.IS_ANDROID) {
            try {
                if (parentClassLoader == null) {
                    asmFactory = new MyASMDeserializerFactory(new ASMClassLoader());
                } else {
                    asmFactory = new MyASMDeserializerFactory(parentClassLoader);
                }
    ...
    public ObjectDeserializer createJavaBeanDeserializer(Class<?> clazz, Type type) {
        ...
        if (!asmEnable) {
            return new MyJavaBeanDeserializer(this, clazz, type);
        }
        ...
        try {
            ...
            return new MyJavaBeanDeserializer(this, beanInfo);
        } catch (Exception e) {
            throw new JSONException("create asm deserializer error, " + clazz.getName(), e);
        }
    }

    public FieldDeserializer createFieldDeserializer(MyParserConfig mapping, //
    ...
    public void addDeny(String name) {
        ...
        long hash = MyTypeUtils.fnv1a_64(name);
    ...
    public Class<?> checkAutoType(String typeName, Class<?> expectClass, int features) {
        ...
        if (autoTypeSupport || expectClass != null) {
            ...
            for (int i = 3; i < className.length(); ++i) {
                ...
                if (Arrays.binarySearch(acceptHashCodes, hash) >= 0) {
                    clazz = MyTypeUtils.loadClass(typeName, defaultClassLoader, false);
                    if (clazz != null) {
                        return clazz;
                    }
                }
                if (Arrays.binarySearch(denyHashCodes, hash) >= 0 && MyTypeUtils.getClassFromMapping(typeName) == null) {
                    throw new JSONException("autoType is not support. " + typeName);
                }
            }
        }
        ...
        if (clazz == null) {
            clazz = MyTypeUtils.getClassFromMapping(typeName);
        }
        ...
        if (clazz != null) {
            if (MyTypeUtils.getAnnotation(clazz,JSONType.class) != null) {
                return clazz;
            }
