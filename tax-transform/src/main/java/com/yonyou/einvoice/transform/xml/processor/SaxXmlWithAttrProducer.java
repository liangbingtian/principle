 ...
 * @author liuqiangm
 */
public class SaxXmlWithAttrProducer extends DefaultHandler {

  Sax2JSONWithAttrHandler jsonWithoutAttrHandler = new Sax2JSONWithAttrHandler();
  ...
  private void preConsume(Object val) {
    if (val instanceof JSONArray) {
      ((JSONArray) val).forEach(v -> preConsume(v));
      return;
    }
    if (!(val instanceof JSONObject)) {
      return;
    }
    JSONArray innerVal = ((JSONObject) val).getJSONArray(Sax2JSONWithAttrHandler.ARR);
    if (innerVal != null && innerVal instanceof JSONArray && ((JSONArray) innerVal).size() == 1
        && !(innerVal.get(0) instanceof JSON)) {
      ((JSONObject) val).remove(Sax2JSONWithAttrHandler.ARR);
      ((JSONObject) val).put(Sax2JSONWithAttrHandler.VALUE, ((JSONArray) innerVal).get(0));
    }
    for (Object tmpVal : ((JSONObject) val).values()) {
      preConsume(tmpVal);
    }
